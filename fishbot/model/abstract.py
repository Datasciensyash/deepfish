...
class VisionModel(torch.nn.Module, abc.ABC):
    ...
    @abc.abstractmethod
    def from_checkpoint(
        cls: tp.Type[VisionModelType], checkpoint_path: Path
    ) -> VisionModelType:
        pass
    ...
    def inference(
        self, input_image: np.ndarray, save_data: bool = True, **kwargs
    ) -> tp.Any:
